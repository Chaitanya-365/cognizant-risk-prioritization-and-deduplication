...
The target used by the Windows Python scanner is:

```text
http://localhost:3000
```

This project should only be used against systems that are owned by the team or where explicit authorization has been provided.
...
This refers to the local OWASP Juice Shop instance.
...
Nuclei will be integrated as another automated scanner.

The Nuclei module should:

1. Run Nuclei against an authorized target.
2. Generate machine-readable JSON output.
3. Automatically retrieve the findings.
4. Parse the JSON findings.
5. Convert the findings into the common vulnerability schema.
6. Pass the normalized findings to the deduplication layer.

The Nuclei module should remain independent from:

- Database
- Dashboard
- Frontend

The scanner should focus only on scanning and producing structured findings.
...
| Nuclei | In Development | Automated Nuclei scanning and JSON ingestion |
...
- [ ] Install/configure Nuclei
- [ ] Automate scanning
- [ ] Retrieve JSON findings
